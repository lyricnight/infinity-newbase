...
public class TargetUtils implements IMinecraft {
    public static PlayerEntity getTarget(double range)
    {
        ...
        return mc.world.getPlayers().stream().filter(Objects::nonNull)
                .filter(LivingEntity::isAlive)
                .filter(entity -> entity != mc.player)
                .filter(entity -> entity.getId() != mc.player.getId())
                .filter(entity -> !Managers.FRIENDS.isFriend(entity))
                .filter(entity -> mc.player.squaredDistanceTo(entity) <= MathUtils.square(range))
