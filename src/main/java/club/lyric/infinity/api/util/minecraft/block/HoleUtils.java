...
import club.lyric.infinity.api.util.minecraft.block.hole.HoleTypes;
...
public class HoleUtils implements IMinecraft {
    ...
    public static boolean isTerrainHole(BlockPos pos) {
        ...
        for (BlockPos off : HOLE_OFFSETS) {
            ...
            if (block == Blocks.STONE || block == Blocks.DIRT || block == Blocks.NETHERRACK || block == Blocks.COBBLESTONE || block == Blocks.GRAVEL || block == Blocks.END_STONE || BlockUtils.isSafe(pos.add(off))) continue;
            return false;
        }
        return true;
    }
